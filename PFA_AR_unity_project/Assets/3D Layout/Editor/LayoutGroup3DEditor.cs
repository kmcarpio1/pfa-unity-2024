...
public class LayoutGroup3DEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        ElementDimensions = EditorGUILayout.Vector3Field(new GUIContent("Element Dimensions", ToolTip_ElemDimensions),
                                                         LayoutGroup.ElementDimensions);
        ...
        else if (Style == LayoutStyle.Grid)
        {
            ...
            SecondaryLayoutAxis =
                (LayoutAxis3D)EditorGUILayout.EnumPopup("Secondary Layout Axis", LayoutGroup.SecondaryLayoutAxis);
        ...
        else if (Style == LayoutStyle.Euclidean)
        {
            ...
            SecondaryConstraintCount =
                EditorGUILayout.IntField("Secondary Constraint Count", LayoutGroup.SecondaryConstraintCount);

            PrimaryAlignment = (Alignment)EditorGUILayout.EnumPopup("Primary Alignment", LayoutGroup.PrimaryAlignment);
            SecondaryAlignment =
                (Alignment)EditorGUILayout.EnumPopup("Secondary Alignment", LayoutGroup.SecondaryAlignment);
            TertiaryAlignment =
                (Alignment)EditorGUILayout.EnumPopup("Tertiary Alignment", LayoutGroup.TertiaryAlignment);
        ...
        {
            LayoutAxis = (LayoutAxis3D)EditorGUILayout.EnumPopup(
                new GUIContent("Plane Normal Axis", ToolTip_Radial_LayoutAxis), LayoutGroup.LayoutAxis);
            SecondaryLayoutAxis = (LayoutAxis3D)EditorGUILayout.EnumPopup(
                new GUIContent("Start Direction", ToolTip_Radial_SecondaryLayoutAxis), LayoutGroup.SecondaryLayoutAxis);
            UseFullCircle = EditorGUILayout.Toggle(new GUIContent("Use Full Circle", ToolTip_Radial_UseFullCircle),
                                                   LayoutGroup.UseFullCircle);
            if (!UseFullCircle)
            {
                MaxArcAngle = EditorGUILayout.FloatField(new GUIContent("Max Arc Angle", ToolTip_Radial_MaxArcAngle),
                                                         LayoutGroup.MaxArcAngle);
            }
            else
            {
            ...
            if (!UseFullCircle)
            {
                RadialSymmetricalAboutStart = EditorGUILayout.Toggle(
                    new GUIContent("Symmetrical Around Start Dir", ToolTip_Radial_SymmAboutStart),
                    LayoutGroup.RadialSymmetricalAboutStart);
            }

            AlignToRadius = EditorGUILayout.Toggle(new GUIContent("Align To Radius", ToolTip_Radial_AlignToRadius),
                                                   LayoutGroup.AlignToRadius);

            if (AlignToRadius)
            {
                PrimaryAlignmentAxis = (LayoutAxis3D)EditorGUILayout.EnumPopup(
                    new GUIContent("Primary Alignment Axis", ToolTip_Radial_PrimaryAlign),
                    LayoutGroup.PrimaryAlignmentAxis);
                SecondaryAlignmentAxis = (LayoutAxis3D)EditorGUILayout.EnumPopup(
                    new GUIContent("Secondary Alignment Axis", ToolTip_Radial_SecondaryAlign),
                    LayoutGroup.SecondaryAlignmentAxis);
            }
    ...
    private void DrawGizmoSettings()
    {
        ...
        if (LayoutGroup.ShowGizmoSettings)
        {
            ...
            GizmoPrimaryColor =
                EditorGUILayout.ColorField(new GUIContent("Primary Gizmo Color"), LayoutGroup.GizmoPrimaryColor);
            GizmoSecondaryColor =
                EditorGUILayout.ColorField(new GUIContent("Secondary Gizmo Color"), LayoutGroup.GizmoSecondaryColor);
            GizmoTertiaryColor =
                EditorGUILayout.ColorField(new GUIContent("Tertiary Gizmo Color"), LayoutGroup.GizmoTertiaryColor);
    ...
    private const string ToolTip_ElemDimensions =
        "The size of child elements in local space used to determine the distance between their pivot points in the layout.  Not used in Radial Layout.";
    ...
    private const string ToolTip_Radial_SecondaryLayoutAxis =
        "The direction of the first element in the radial layout.  Must be orthogonal to Plane Normal.";
    private const string ToolTip_Radial_UseFullCircle =
        "If true, angle between elements is automatically calculated so that they evenly fill a circle";
    private const string ToolTip_Radial_MaxArcAngle =
        "Elements will be distributed around the circle between 0 and this angle";
    private const string ToolTip_Radial_AlignToRadius =
        "If true, Elements will be rotated to align one axis pointing outward from the circle center, and another axis aligned with the plane normal";
    private const string ToolTip_Radial_PrimaryAlign =
        "The transform axis of the elements that should point outward from the center of the circle.";
    private const string ToolTip_Radial_SecondaryAlign =
        "The transform axis of the elements that should point in the same direction as the plane normal.";
    private const string ToolTip_Radial_SymmAboutStart =
        "If enabled, the elements will be distributed symmetrically around the starting direction.  Otherwise, elements will be distributed starting from the start direction.";
}
}
