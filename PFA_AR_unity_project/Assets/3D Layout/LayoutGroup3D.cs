...
public class LayoutGroup3D : MonoBehaviour
{
    ...
    public void LinearLayout()
    {
        ...
        Vector3 alignmentOffset =
            LayoutElements[LayoutElements.Count - 1].localPosition - LayoutElements[0].localPosition;
...
#region Calculate alignment offset vectors
    ...
    public void RadialLayout()
    {
        ...
        {
            float angle =
                (LayoutElements.Count > 1) ? (float)i / (LayoutElements.Count - 1) * MaxArcAngle * Mathf.Deg2Rad : 0f;
    ...
    private void DrawRadialGizmos()
    {
        ...
        DrawGizmoArrow(origin, origin + worldNormal * lineLength * GizmoScale,
                       Vector3.Cross(worldNormal, transform.TransformDirection(primaryAxis)));
        ...
        for (int i = 1; i < arcPoints; i++)
        {
            ...
            DrawGizmoArrow(transform.TransformPoint(arcPos), transform.TransformPoint(pos), worldNormal,
                           0.5f * Mathf.Clamp01(GizmoScale));
